...
import repackaged.com.sun.midp.io.ConnectionBaseAdapter;
import repackaged.com.sun.midp.io.DateParser;
import repackaged.com.sun.midp.io.Properties;
import repackaged.com.sun.midp.io.j2me.http.StreamConnectionElement;
import repackaged.com.sun.midp.io.j2me.http.StreamConnectionPool;
 ...
 * @author mariotaku
 */
public class SocketHttpConnection extends ConnectionBaseAdapter
		implements HttpConnection {

	/** HTTP version string to use with all outgoing HTTP requests. */
	protected static final String HTTP_REQUEST_VERSION = "HTTP/1.1";
	...
	static {
		 ...
		 */
		String flag = System.getProperty("com.sun.midp.io.http.force_non_persistent");
		 ...
		 */
		prop = System.getProperty("com.sun.midp.io.http.max_persistent_connections");
		if (prop != null) {
			try {
		...
		prop = System.getProperty("com.sun.midp.io.http.persistent_connection_linger_time");
		if (prop != null) {
			try {
				temp = Integer.parseInt(prop);
				if (temp >= 0) {
		 ...
		 * from the configuration file.
		 */
		prop = System.getProperty("com.sun.midp.io.http.inputBufferSize");
		if (prop != null) {
			try {
				temp = Integer.parseInt(prop);
				if (temp <= 0) {
		...
		prop = System.getProperty("com.sun.midp.io.http.outputBufferSize");
		if (prop != null) {
			try {
				temp = Integer.parseInt(prop);
				if (temp <= 0) {
		...
		outputDataSize = outputBufferSize - HTTP_OUTPUT_DATA_OFFSET - HTTP_OUTPUT_EXTRA_ROOM;
	}
	/** Parsed Url. */
	...
	public void setSSLErrorsIgnored(boolean sslErrorsIgnored) {
	...
	/**
	 * Provides the connect() method that sets up the connection, but
	...
	private void streamConnect() throws IOException {
		...
		streamConnection = connectionPool.get(url.getScheme(), url.getHost(), url.getPort());
	...
	private void sendRequestHeader() throws IOException {
		...
		if (url.getQuery() != null) {
			reqLine.append("?");
			reqLine.append(url.getQuery());
		}

		reqLine.append(" ");
		reqLine.append(HTTP_REQUEST_VERSION);
		reqLine.append("\r\n");
		 ...
		 */
		if (!reqProperties.hasKey("Host")) {
			setRequestField("Host", url.getAuthority());
		}
	...
	protected StreamConnection connect() throws IOException {
		...
		if ("http".equals(protocol)) {
			return (StreamConnection) Connector.open("socket://" + host + ":" + (port < 0 ? 80 : port));
		} else if ("https".equals(protocol)) {
			if (sslErrorsIgnored) {
				return UnsafeSSLConnection.open(host, port < 0 ? 443 : port);
			} else {
				return (StreamConnection) Connector.open("ssl://" + host + ":" + (port < 0 ? 443 : port));
			}
		}
	...
	protected void disconnect() throws IOException {
		...
		if (!connectionPool.add(url.getScheme(), url.getHost(), url.getPort(),
