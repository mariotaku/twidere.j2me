...
import org.sun.midp.crypto.MessageDigest;
import org.sun.midp.crypto.RSAPublicKey;
import org.sun.midp.log.Logging;
import org.sun.midp.log.LogChannels;
...
public class X509Certificate implements Certificate {
	...
	public X509Certificate(byte ver, byte[] rawSerialNumber, String sub,
			String iss, long notBefore, long notAfter, byte[] mod, byte[] exp, byte[] chash, int pLen)
			throws Exception {
		version = ver;
	...
	private void parseExtensions(int end) throws IOException {
		...
		while (idx < end) {
			...
			if (Logging.REPORT_LEVEL <= Logging.INFORMATION) {
				Logging.report(Logging.INFORMATION, LogChannels.LC_SECURITY, "<Id: " + Utils.hexEncode(enc, extIdIdx, extIdLen) + (crit ? ", critical, " : ", ") + Utils.hexEncode(enc, extValIdx, extValLen) + ">" + ((extId == null) ? " (Unrecognized)" : ""));
			}
	...
	public byte[] getFingerprint() {
		...
		if (fp != null) {
			System.arraycopy(fp, 0, res, 0, res.length);
		}
		return res;
	}
	...
	public void verify(PublicKey k) throws CertificateException {
		...
		try {
			...
			throw new CertificateException(ex.getMessage(), this,
			...
			throw new CertificateException(e.getMessage(), this,
					CertificateException.VERIFICATION_FAILED);
