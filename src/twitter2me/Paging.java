...
public final class Paging {
	...
	private void addPostParameter(final char[] supportedParams, final char paramKey,
			final List pagingParams, final String paramName, final long paramValue) {
		boolean supported = false;
		final int length = supportedParams != null ? supportedParams.length : 0;
		for (int i = 0; i < length; i++) {
			if (supportedParams[i] == paramKey) {
				supported = true;
				break;
			}
		}
	...
	HttpParameter[] asPostParameterArray() {
		final List list = asPostParameterList(SMCP, COUNT);
		if (list.size() == 0) {
	...
	HttpParameter[] asPostParameterArray(final char[] supportedParams, final String perPageParamName) {
		final List pagingParams = new ArrayList(supportedParams.length);
	...
	/* package */
	List asPostParameterList(final char[] supportedParams) {
	 ...
	 */
	/* package */
	List asPostParameterList(final char[] supportedParams, final String perPageParamName) {
		final List pagingParams = new ArrayList(supportedParams.length);
		addPostParameter(supportedParams, 's', pagingParams, "since_id", getSinceId());
		addPostParameter(supportedParams, 'm', pagingParams, "max_id", getMaxId());
		addPostParameter(supportedParams, 'c', pagingParams, perPageParamName, getCount());
