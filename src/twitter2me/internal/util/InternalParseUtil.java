...
public final class InternalParseUtil {
	...
	public static boolean getBoolean(final String name, final JSONObject json) {
		...
		return BooleanUtil.parseBoolean(str);
	}
	...
	public static Date getDate(final String dateStr) throws TwitterException {
		return new Date(getTimestamp(dateStr));
	}

	public static long getTimestamp(final String dateStr) throws TwitterException {
		try {
			return DateParser.parse(dateStr);
		} catch (final IllegalArgumentException e) {
			throw new TwitterException("Unexpected date format(" + dateStr + ") returned from twitter.com", e);
		}
	}
	...
	public static int getInt(final String str) {
		...
		try {
			...
			// http://twitter2me.org/jira/browse/TFJ-484
			return -1;
		}
	}

	public static int getDouble(final String str) {
		if (TextUtils.isEmpty(str)) {
			return 0;
		}
		try {
	...
	public static int getDouble(final String name, final JSONObject json) {
		return getDouble(getRawString(name, json));
	}
	...
	private static String replace(final String str, final String pattern, final String replacement) {
		return InternalStringUtil.replace(str, pattern, replacement);
	}
}
